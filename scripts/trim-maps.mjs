...
const DENSITY_RATIO = 0.5 // first candidate: row with ≥50% dark pixels
...
async function findMapBounds(inputPath) {
  ...
  const { data } = await img.clone().grayscale().raw().toBuffer({ resolveWithObject: true })
  ...
  outer: for (let x = 0; x < width; x++) {
    for (let y = top; y <= bottom; y++) {
      if (data[y * width + x] <= BG_THRESHOLD) {
        left = x
        break outer
      }
    }
