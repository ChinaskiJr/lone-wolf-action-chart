...
import { MAGNAKAI_DISCIPLINES, KAI_WEAPONS, getDisciplineMap } from '@/data/disciplines'
...
export function DisciplinesPanel() {
  ...
  const {
    character,
    addDiscipline,
    setWeaponskillWeapon,
    addWeaponmasteryWeapon,
    removeWeaponmasteryWeapon,
  } = useCharacterStore()
  ...
  const completedCircles = character.cycle === 'magnakai' ? getCompletedCircles(selected) : []
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {character.cycle === 'kai' && selected.includes('weaponskill') && (
        <div className="bg-blue-950/20 border border-blue-900/40 rounded-lg p-3 text-sm">
          <span className="text-slate-400">
            {lang === 'fr' ? 'Arme maîtrisée' : 'Mastered weapon'} :{' '}
          </span>
          <span className="text-blue-300 font-medium">
            {(() => {
              const w = KAI_WEAPONS.find(
                (ww) => ww.key === (character as KaiCharacter).weaponskillWeapon
              )
              return w
                ? lang === 'fr'
                  ? w.fr
                  : w.en
                : (character as KaiCharacter).weaponskillWeapon
            })()}
          </span>
          <span className="text-slate-500 ml-2 text-xs">
            (+2 HC {lang === 'fr' ? 'si portée' : 'if carried'})
          </span>
        </div>
      )}
      ...
      {(character.cycle === 'magnakai' ||
        ...
        selected.includes(
          character.cycle === 'magnakai' ? 'weaponmastery' : 'grandWeaponmastery'
        ) &&
        (() => {
          const owned: string[] =
          ...
          const max =
            character.cycle === 'magnakai' ? character.currentBook - 3 : character.currentBook - 12
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {LORE_CIRCLES.map((circle) => {
              ...
              return (
                <div
                  key={circle.id}
                  className={`rounded-lg border p-3 ${isComplete ? 'border-green-700 bg-green-950/20' : 'border-slate-800 bg-slate-800/20'}`}
                >
                  <div
                    className={`font-medium text-sm ${isComplete ? 'text-green-300' : 'text-slate-400'}`}
                  >
                    {isComplete && '✓ '}
                    {lang === 'fr' ? circle.fr : circle.en}
                  </div>
                  <div className="text-xs text-slate-500 mt-1">
                    {ownedCount}/{circle.disciplines.length} disciplines
                    {isComplete && (
                      <span className="text-green-500 ml-2">
                        +{circle.bonusCS} HC +{circle.bonusEP} PE
                      </span>
                    )}
                  </div>
                </div>
      ...
      {character.cycle === 'grandmaster' &&
        (character as GrandMasterCharacter).magnakaiDisciplines?.length > 0 && (
          <div className="bg-green-950/10 border border-green-900/30 rounded-lg p-3">
            <div className="text-xs text-green-600 font-medium mb-1.5">
              Disciplines Magnakaï conservées
            </div>
            <div className="flex flex-wrap gap-1.5">
              {(character as GrandMasterCharacter).magnakaiDisciplines.map((dk) => {
                const d = MAGNAKAI_DISCIPLINES[dk as keyof typeof MAGNAKAI_DISCIPLINES]
                return d ? (
                  <span
                    key={dk}
                    className="text-xs px-2 py-0.5 rounded-full bg-green-900/30 text-green-400 border border-green-900/50"
                  >
                    {lang === 'fr' ? d.fr : d.en}
                  </span>
                ) : null
