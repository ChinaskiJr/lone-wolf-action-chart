...
export function StatsPanel() {
  ...
  const { bonusCS: circleCS, bonusEP: circleEP } =
  ...
  const epBarColor =
    epPercent > 50 ? 'bg-green-600' : epPercent > 25 ? 'bg-yellow-600' : 'bg-red-600'
  const epTextColor =
    epPercent > 50 ? 'text-green-400' : epPercent > 25 ? 'text-yellow-400' : 'text-red-400'
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          ...
          <div className="bg-slate-900/60 rounded-lg p-3">
            <div className="text-3xl font-bold text-blue-400 tabular-nums">
              {character.combatSkill.bonus >= 0 ? '+' : ''}
              {character.combatSkill.bonus}
            </div>
            ...
            {weaponsHC > 0 && (
              <div className="mt-2 text-xs text-slate-500 leading-tight">
                {totalCS - itemsHC - weaponsHC}{' '}
                <span className="text-blue-400 font-semibold">+{weaponsHC}</span>
            ...
            {itemsHC !== 0 && (
              <div className="mt-1 text-xs text-slate-500 leading-tight">
                {totalCS - itemsHC}{' '}
                <span className={`font-semibold ${itemsHC > 0 ? 'text-blue-400' : 'text-red-400'}`}>
                  {itemsHC > 0 ? `+${itemsHC}` : itemsHC}
                </span>
            ...
            {circleCS > 0 && (
              <div className="mt-1 text-xs text-slate-500 leading-tight">
                {totalCS - circleCS}{' '}
                <span className="font-semibold text-amber-400">+{circleCS}</span>
          ...
          <div className="bg-amber-900/30 border border-amber-800/50 rounded-lg p-3 flex flex-col items-center justify-center">
            <div className="text-3xl md:text-5xl font-bold text-amber-400 tabular-nums">
              {totalCS}
            </div>
          ...
          <div className="text-right">
            ...
            {itemsPE !== 0 && (
              <div className="mt-1 text-xs text-slate-600 leading-tight">
                {maxEP - itemsPE}{' '}
                <span
                  className={`font-semibold ${itemsPE > 0 ? 'text-green-400/80' : 'text-red-400'}`}
                >
                  {itemsPE > 0 ? `+${itemsPE}` : itemsPE}
                </span>
            ...
            {circleEP > 0 && (
              <div className="mt-1 text-xs text-slate-600 leading-tight">
                {maxEP - circleEP}{' '}
                <span className="font-semibold text-amber-400/80">+{circleEP}</span>
