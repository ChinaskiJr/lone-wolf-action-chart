...
function characterHasHunting(char: Character): boolean {
  switch (char.cycle) {
    ...
    case 'grandmaster':
      return (
        char.disciplines.includes('grandHuntmastery') ||
        char.magnakaiDisciplines.includes('huntmastery')
      )
    case 'neworder':
...
function BackpackSection({
  ...
  const [confirmDelete, setConfirmDelete] = useState<
    { type: 'meal' } | { type: 'item'; id: string; label: string } | null
  >(null)
  ...
  function confirmEdit(item: BackpackItem) {
    ...
    if (item.epRestore != null) {
      ...
      onUpdate(item.id, {
        ...
        slots: editTwoSlots ? 2 : undefined,
      })
    }
    setEditingId(null)
  }
  ...
  for (const item of items) {
    ...
    const type = item.csBonus
      ? ('combatPotion' as const)
      : item.epRestore
        ? ('potion' as const)
        : ('item' as const)
    ...
    <div>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-slate-500 truncate">
          {meals === 0 ? t('sheet.noFoodPenalty') : hasHunting ? t('sheet.huntingHint') : ''}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 mb-3">
        {slotRows.map((slot, i) => {
          if (slot.type === 'meal')
            return (
              ...
              >
                ...
                <button
                  onClick={() => setConfirmDelete({ type: 'meal' })}
          ...
          if (slot.type === 'potion') {
            if (editingId === slot.item.id)
              return (
                <div
                  key={slot.item.id}
                  className="flex items-center gap-1.5 rounded-lg px-3 py-2 border border-blue-700/50 bg-blue-950/20 col-span-full"
                >
                  <span className="text-xs text-slate-600 w-6 shrink-0">
                    {slotLabel(slot.startSlot, slot.endSlot)}
                  </span>
                  <span className="shrink-0">🧪</span>
                  <input
                    autoFocus
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') confirmEdit(slot.item)
            ...
            return (
              ...
              >
                ...
                <button
                  onClick={() =>
                    setConfirmDelete({ type: 'item', id: slot.item.id, label: slot.item.name })
                  }
                  aria-label={t('sheet.removeItem')}
                  className="relative text-slate-600 hover:text-red-400 transition-colors shrink-0 before:absolute before:inset-[-10px]"
                >
          ...
          if (slot.type === 'combatPotion') {
            if (editingId === slot.item.id)
              return (
                <div
                  key={slot.item.id}
                  className="flex items-center gap-1.5 rounded-lg px-3 py-2 border border-violet-700/50 bg-violet-950/20 col-span-full"
                >
                  <span className="text-xs text-slate-600 w-6 shrink-0">
                    {slotLabel(slot.startSlot, slot.endSlot)}
                  </span>
          ...
          if (slot.type === 'item') {
            if (editingId === slot.item.id)
              return (
                <div
                  key={slot.item.id}
                  className="flex flex-col gap-1.5 rounded-lg px-3 py-2 border border-amber-700/50 bg-slate-800/60 col-span-full"
                >
            ...
            return (
              ...
              >
                <span className="text-xs text-slate-600 w-6 shrink-0 mt-0.5">
                  {slotLabel(slot.startSlot, slot.endSlot)}
                </span>
                ...
                <button
                  onClick={() => startEdit(slot.item)}
                  aria-label={t('sheet.editItem')}
                  className="relative text-slate-600 hover:text-slate-400 transition-colors shrink-0 mt-0.5 before:absolute before:inset-[-10px]"
                >
                  <Pencil size={12} />
                </button>
                <button
          ...
          return (
            <div
              key={`empty-${i}`}
              className="flex items-center gap-2 rounded-lg px-3 py-2 border border-slate-800/60 bg-slate-900/30"
            >
              <span className="text-xs text-slate-600 w-6 shrink-0">{slot.startSlot}</span>
          ...
          <div className="bg-slate-900 border border-red-900/60 rounded-xl p-6 max-w-sm w-full shadow-xl">
            <p className="text-sm text-slate-300 mb-5">
              ...
              {confirmDelete.type === 'item' && (
                <span className="font-semibold text-slate-100"> {confirmDelete.label}</span>
              )}
              {confirmDelete.type === 'meal' && (
                <span className="font-semibold text-slate-100"> {t('sheet.meals')}</span>
              )}
            </p>
