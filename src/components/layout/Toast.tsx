...
export function Toast({ message, variant = 'info', action, onDismiss, duration = 5000 }: Props) {
  ...
  return (
    <div className={`fixed bottom-4 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-3 px-4 py-3 rounded-xl border shadow-xl max-w-sm w-[calc(100vw-2rem)] animate-victory ${VARIANT_CLASSES[variant]}`}>
      ...
      <button onClick={onDismiss} aria-label="Fermer" className="shrink-0 opacity-60 hover:opacity-100 transition-opacity">
        <X size={14} />
      </button>
    </div>
  )
