        ...
        {weapons.map((w, i) => {
          ...
          if (editingIndex === i)
            return (
              <div
                key={i}
                className="flex items-center gap-2 bg-slate-800/60 border border-amber-700/50 rounded-lg px-3 py-2"
              >
                <div className="w-5 h-5 shrink-0 rounded bg-amber-800/40 flex items-center justify-center text-xs text-amber-500">
                  ⚔
                </div>
                <input
                  autoFocus
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') confirmEdit(i, w)
          ...
          return (
            <div
              key={i}
              className={`flex items-center gap-2 bg-slate-800/60 border border-slate-700 rounded-lg px-3 py-2.5 transition-opacity ${isEquipped ? '' : 'opacity-50'}`}
            >
              <label
              ...
              {w.bonus != null && (
                <BonusBadge value={w.bonus} kind="hc" active={isEquipped} dimWhenInactive={false} />
              )}
              <button
                onClick={() => startEdit(i, w)}
                aria-label={t('sheet.editItem')}
                className="relative text-slate-600 hover:text-slate-400 transition-colors before:absolute before:inset-[-10px]"
              >
                <Pencil size={12} />
              </button>
              <button
        ...
        <div className="flex items-center gap-3 px-3 py-2 mt-1 mb-2">
          ...
          {hasQuiver && (
            <div className="flex items-center gap-2 ml-auto">
              ...
              <span className="tabular-nums text-sm text-slate-200 min-w-[1.5rem] text-center">
                {arrows}
              </span>
              <button
                onClick={() => onSetArrows(arrows + 1)}
          ...
          <div className="bg-slate-900 border border-red-900/60 rounded-xl p-6 max-w-sm w-full shadow-xl">
            <p className="text-sm text-slate-300 mb-5">
              ...
              <span className="font-semibold text-slate-100">
                {weapons[confirmDeleteIndex]?.name}
              </span>
            </p>
