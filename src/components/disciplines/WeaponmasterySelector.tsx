      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-slate-400">
          ...
          <span className="text-slate-600 ml-1">
            (+{hcBonus} HC {lang === 'fr' ? 'si portée' : 'if carried'})
          </span>
        </span>
      ...
      <div className="flex flex-wrap gap-1.5 min-h-[24px]">
        {owned.map((key) => {
          ...
          return (
            <span
              key={key}
              className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-900/40 border border-blue-800/60 text-xs text-blue-300"
            >
              {w ? (lang === 'fr' ? w.fr : w.en) : key}
              <button
                type="button"
                onClick={() => onRemove(key)}
                className="text-blue-500 hover:text-blue-200 ml-0.5"
              >
                ×
              </button>
            </span>
