        ...
        {items.map(item => {
          ...
          return (
            <div key={item.id} className="flex gap-2 bg-amber-950/20 border border-amber-900/30 rounded-lg px-3 py-2">
              ...
              <button onClick={() => startEdit(item)} aria-label={t('sheet.editItem')} className={`relative text-slate-600 hover:text-slate-400 transition-opacity shrink-0 mt-0.5 before:absolute before:inset-[-10px] ${isEquipped ? '' : 'opacity-50'}`}>
                <Pencil size={12} />
              </button>
              <button onClick={() => setConfirmDelete({ type: 'item', id: item.id, name: item.name })} aria-label={t('sheet.removeItem')} className={`relative text-slate-600 hover:text-red-400 transition-opacity shrink-0 mt-0.5 before:absolute before:inset-[-10px] ${isEquipped ? '' : 'opacity-50'}`}>
                <X size={13} />
              </button>
            </div>
          )
