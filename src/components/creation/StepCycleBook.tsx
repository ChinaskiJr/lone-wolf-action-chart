...
const CYCLE_INFO: { id: Cycle; books: string; color: string; activeBg: string }[] = [
  { id: 'kai', books: '1–5', color: 'border-blue-700 hover:border-blue-500', activeBg: 'bg-blue-900/50 border-blue-500' },
  { id: 'magnakai', books: '6–12', color: 'border-green-700 hover:border-green-500', activeBg: 'bg-green-900/50 border-green-500' },
  { id: 'grandmaster', books: '13–20', color: 'border-amber-700 hover:border-amber-500', activeBg: 'bg-amber-900/50 border-amber-500' },
  { id: 'neworder', books: '21–32', color: 'border-red-700 hover:border-red-500', activeBg: 'bg-red-900/50 border-red-500' },
]
...
export function StepCycleBook({ initialCycle, initialBook, onNext }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h2 className="text-xl font-serif font-semibold text-amber-100">{t('creation.chooseCycle')}</h2>
      ...
      <div className="grid grid-cols-2 gap-3">
        {CYCLE_INFO.map(({ id, books, color, activeBg }) => (
          ...
          >
            ...
            <div className="text-xs text-slate-400 mt-0.5">{t('home.book')} {books}</div>
          </button>
        ))}
      </div>

      <div>
        <label className="block text-sm font-medium text-slate-300 mb-2">{t('creation.chooseBook')}</label>
        <select
          value={book}
      ...
      {cycle === 'magnakai' && (
        <div className="text-xs text-amber-700 bg-amber-950/30 border border-amber-900 rounded p-3">
          ⚠ Démarrer au cycle Magnakaï sans avoir joué le cycle Kaï. Vos disciplines Kaï seront vides.
        </div>
      )}
